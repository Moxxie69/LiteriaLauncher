    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            setOverlayContent(
                ...
                'L\'authentification Microsoft à échouée. Veuillez reéssayer.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                .catch((displayableError) => {
                    ...
                    if(isDisplayableError(displayableError)) {
                        ...
                        // Uh oh.
                        msftLoginLogger.error('Erreur durant la connexion ', displayableError)
                        actualDisplayableError = {
                            title: 'Erreur inconnue durant la connexion',
                            desc: 'Une érreur est survenue, veuillez checker la console pour plus de détails.'
                        }
                    }
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le launcher il faut être connecter à au moins UN compte. Vous allez devoir vous connecter.<br><br>êtes vous sur de vouloir faire ça?',
                    'Sur et certain !',
                    'En faite non..'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Oula..',
                'Microsoft n\'a pas pu vous déconnecter',
                'ah.'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Sélectionné &#10004;' : '>Sélectionner compte'}</button>
...
function populateJavaReqDesc(server) {
    settingsJavaReqDesc.innerHTML = `Java ${server.effectiveJavaOptions.suggestedMajor} x64 Requis.`
}
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/Moxxie69/LiteriaLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Erreur pendant le chargement des notes.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle ${isPrerelease(data.version) ? 'BETA' : 'Release'} Disponible`
        ...
        settingsUpdateButtonStatus('Rechercher les MAJs', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Recherche des MAJs..', true)
            }
        })
    }
}
